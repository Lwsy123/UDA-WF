...
path = "/home/siyuwang/pythoncode/TranferLearning/Dataset/AWF_100.npz"
labledeck = dict()
...
dataset_y = dataset_y[index]


   

WF_x = [np.empty([0, 5000],dtype=int)]*5 # 得到空数据
WF_y = [np.empty([0, labels.shape[0]],dtype=int)]*5
...
for i in range(len(labels)):
    ...
    for col, j in enumerate([5, 10, 15, 20]):
        index_select = np.random.choice(index, j, replace=False)
        x = dataset_x[index_select] # 得到当前的数据
        y = labels[np.ones(j, dtype=int) * i] # 得到当前的label
        WF_x[col] = np.append(WF_x[col], x, axis=0) # 追加数据 
        WF_y[col] = np.append(WF_y[col], y, axis=0) # 追加标签数据
        print(WF_x[col].shape, WF_y[col].shape)

for col, j in enumerate([ 5, 10, 15, 20]):
    np.savez("./Dataset/AWF/AWF" + str(j), x = WF_x[col], y = WF_y[col])  
